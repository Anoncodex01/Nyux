...
export function ActuarialDropdown({ onServiceClick }: { onServiceClick?: () => void }) {
  return (
    <div className="w-full md:min-w-[260px] md:border-l border-[#f7e7d6] px-2 md:px-4 py-2 md:py-0">
      ...
      <ul className="space-y-2">
        {actuarialItems.map(item => (
          <li key={item.label}>
            <Link href={item.href} className="text-gray-800 hover:text-[#953735] cursor-pointer transition-colors text-sm md:text-base pl-2 py-1 block" onClick={onServiceClick}>
              {item.label}
            </Link>
          </li>
        ))}

        ...
        <nav className="hidden md:flex gap-6 items-center text-sm font-medium text-black relative">
          ...
          >
            ...
            {servicesOpen && (
              ...
              >
                <ServicesDropdown onServiceClick={() => setServicesOpen(false)} />
              </div>
            )}
          </div>
      ...
      {open && (
        <div className="fixed inset-0 z-40 bg-black/40 flex md:hidden" onClick={() => setOpen(false)}>
          ...
          >
            ...
            {/* Collapsible Our Services */}
            <button
              className="flex items-center gap-2 hover:text-[#964604] focus:outline-none"
              onClick={() => setMobileServicesOpen(v => !v)}
              aria-expanded={mobileServicesOpen}
            >
              Our Services
              <svg className={`w-4 h-4 ml-1 transition-transform duration-200 ${mobileServicesOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" /></svg>
            </button>
            {mobileServicesOpen && (
              <div className="pl-0 mt-2 mb-2 w-full">
                <ServicesDropdown mobile={true} onServiceClick={() => { setMobileServicesOpen(false); setOpen(false); }} />
              </div>
            )}
